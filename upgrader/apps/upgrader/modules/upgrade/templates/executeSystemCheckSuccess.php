            ...
            <?php
               ...
               if(function_exists('mysqli_connect') && ($conn = @mysqli_connect($dbInfo['host'], $dbInfo['username'], $dbInfo['password'], "", $dbInfo['port']))) {
	              ...
	              $mysqlServer = mysqli_get_server_info($conn);
                        ...
                        if ($systemValidator->isMySqlCompatible($dbInfo['host'], $dbInfo['username'], $dbInfo['password'], $dbInfo['dbHostPortModifier'], $dbInfo['dbHostPort'])) {
                            echo "<b><font color='green'>OK (ver " . $mysqlServer . ')</font></b>';
                        } else {
                            $mysqlErrorMessage = $systemValidator->getMysqlErrorMessage($dbInfo['host'], $dbInfo['username'], $dbInfo['password'], $dbInfo['dbHostPortModifier'], $dbInfo['dbHostPort']);
