...
function getDbConn($dbInfo) {
    if ($dbInfo['dbHostPortModifier'] == 'socket') {
        return mysqli_connect($dbInfo['dbHostName'], $dbInfo['dbUserName'], $dbInfo['dbPassword'], null, null, $dbInfo['dbHostPort']);
    } else {
          ...
          <tr>
            ...
            <?php
               ...
               if(function_exists('mysqli_connect') && ($conn = getDbConn($dbInfo))) {
	              ...
	              if($systemValidator->isMySqlCompatible($dbInfo['dbHostName'], $dbInfo['dbUserName'], $dbInfo['dbPassword'], $dbInfo['dbHostPortModifier'], $dbInfo['dbHostPort'])) {
                  	 echo "<b><font color='green'>OK (ver " .$mysqlServer. ')</font></b>';
                  } else {
                       $error_found = true;
                       $mysqlErrorMessage = $systemValidator->getMysqlErrorMessage($dbInfo['dbHostName'], $dbInfo['dbUserName'], $dbInfo['dbPassword'], $dbInfo['dbHostPortModifier'], $dbInfo['dbHostPort']);
